...
def send_birthday_wish():
	...
	# if int(frappe.db.get_single_val  ue("HR Settings", "stop_birthday_reminders") or 0):
	# 	return
	...
	if birthdays:
		...
		for e in birthdays:
			...
			frappe.sendmail(recipients=['maharaja.s@groupteampro.com'],
...
def update_expense_approval(doc,status,approval_status):
	frappe.db.set_value("Expense Claim",doc,"workflow_state",status)
	frappe.db.set_value("Expense Claim",doc,"approval_status",approval_status)
	...
	# })
	# tm.save(ignore_permissions=True)
	# frappe.log_error(title='tm',message=tm)
...
@frappe.whitelist()
def check_before_application(employee,from_date,to_date):
	query = """select name,employee,attendance_date,status from `tabLeave Application` where employee=%s and attendance_date between '%s' and '%s' """ % (employee,from_date,to_date)
	attendance = frappe.db.sql(query,as_dict=True)
	for a in attendance:
		doc = frappe.get_doc("Attendance",a.name)
		doc.update({
...
def create_ot():
	Attendance = frappe.get_all("Attendance",{"employee":"TP001"},['*'])
	for att in Attendance:
		if(att.out_time):
			intime = datetime.strptime(att.in_time, '%Y-%m-%d %H:%M:%S')
			outtime = datetime.strptime(att.out_time, '%Y-%m-%d %H:%M:%S')
			time_delta = outtime -intime
			frappe.db.set_value("Attendance",att.name,"overtime",time_delta)
			# extratime = datetime.strptime('10:00:00', '%H:%M:%S').time()
			# actualtime = datetime.strptime('08:00:00', '%H:%M:%S').time()
			extra =timedelta(hours=10, seconds=0)    
			actual = timedelta(hours=8, seconds=0)
			# print(time_delta)
			# print(extra)
			if(time_delta >= extra):
				# print("hi")
				ot =time_delta - actual
				# ot_time = datetime.strptime('08:00:00', '%H:%M:%S').strftime('%H:%M')
				ftr =[3600,60,1]
				hr =sum([a*b for a,b in zip(ftr,map(int,str(ot).split(':')))])
				ot_time =hr/3600
				print(ot_time)
				print(att.name)
				frappe.db.set_value("Attendance",att.name,"ot",ot_time)
			else:
				frappe.db.set_value("Attendance",att.name,"ot",0)
				
@frappe.whitelist()
def check_leaveapplication_status(from_date,employee,holiday_list):
	holiday = frappe.get_doc("Holiday List",holiday_list)
	for t in holiday.get("holidays"):
		date = datetime.strptime(from_date, '%Y-%m-%d').date()
		yesterday = date - timedelta(days=1)
		day_before_yesterday = date - timedelta(days=2)
		before_two_days = date - timedelta(days=3)
		before_three_days = date - timedelta(days=4)
		before_four_days = date - timedelta(days=5)
		tomorrow = date + timedelta(days=1)
		day_after_tomorrow = date + timedelta(days=2)
		after_two_days = date + timedelta(days=3)
		after_three_days = date + timedelta(days=4)
		after_four_days = date + timedelta(days=5)
		frappe.errprint(day_after_tomorrow)
		if t.holiday_date == yesterday:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":day_before_yesterday}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(day_before_yesterday))
		elif t.holiday_date == yesterday or day_before_yesterday:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":before_two_days}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(before_two_days))
		elif t.holiday_date == yesterday or day_before_yesterday or before_two_days:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":before_three_days}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(before_three_days))
		elif t.holiday_date == yesterday or day_before_yesterday or before_two_days or before_three_days:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":before_four_days}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(before_four_days))

		if t.holiday_date == tomorrow:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":day_after_tomorrow}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(day_after_tomorrow))
		elif t.holiday_date == tomorrow or day_after_tomorrow:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":after_two_days}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(after_two_days))
		elif t.holiday_date == tomorrow or day_after_tomorrow or after_two_days:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":after_three_days}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(after_three_days))
		elif t.holiday_date == tomorrow or day_after_tomorrow or after_two_days or after_three_days:
			if frappe.db.exists("Leave Application",{"employee": employee, "to_date":after_four_days}):
				frappe.throw(_('You have applied leave on {0} ,Kindly include holidays also').format(after_four_days))
...
@frappe.whitelist()
def get_emp_id():
	emp_name = frappe.db.get_value('Employee',{'user_id':frappe.user.session},['employee'])
	return emp_name


# @frappe.whitelist()
# def get_emp():
#     name = frappe.db.sql("""select employee_code1 from `tabPerformance Management Manager` where appraisal_year = '2022' """,as_dict=1)
#     for n in name:
#         print(n.employee_code1)
...
#     ]
#     for e in emp:
#         det = frappe.db.sql("""select employee_code1 from `tabPerformance Management Self` where appraisal_year = '2022' and docstatus = 1 and employee_code1 ='%s' """%(e),as_dict=1)
#         for d in det:
#             emp_det = frappe.db.sql("""select employee_code1 from `tabPerformance Management Manager` where appraisal_year = '2022' and employee_code1 ='%s' """%(d.employee_code1),as_dict=1)
#             for di in emp_det: 
#                 print(di.employee_code1)

# @frappe.whitelist()
# def update_doc():
#     doc = frappe.db.sql("""update `tabPerformance Management Calibration` set r_location = location where appraisal_year = 2022 """)

# def update_leave():
#     print("hi")
#     l = frappe.new_doc("Leave Application")
#     l.employee = "1009"
#     l.from_date = "2023-07-05"
#     l.to_date = "2023-07-05"
...
# @frappe.whitelist()
# def update_leave():
#     print("Updating leave applications...")
    
#     sql_query = """
#         UPDATE `tabLeave Application`
#         SET docstatus = 1
#         WHERE from_date BETWEEN '2023-01-01' AND '2023-12-31'
#         and status = 'Approved' """
    
#     try:
#         leave = frappe.db.sql(sql_query, as_dict=1)
#         print("Number of records updated:", len(leave))
#         frappe.db.commit()
...
def update_employee_no(name,employee_number):
	...
	for i in emps:
		if emp.employee_number == employee_number:
			...
			frappe.throw(_("Employee Number already exists for {i.name}"))
		else:
