    ...
    for att in attendance:
        ...
        if holiday_date:
            for h in holiday_date:
                # get_continuous_absents(att.employee,att.attendance_date)
...
def validate_if_attendance_not_applicable(employee, attendance_date):
    # Check if attendance is Preseent
    ...
    if tm_record:
        return True 
    # admin_approved_status = frappe.db.sql("""select admin_approved_status from `tabAttendance`
    #         where employee = %s and %s between from_date and to_date
    #         and docstatus = 1""", (employee, attendance_date), as_dict=True)
